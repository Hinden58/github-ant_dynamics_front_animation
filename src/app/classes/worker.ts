...
export class Worker extends Ant { 
  ...
  _supply_max:number;
  constructor(element:number,x:number,y:number, origin:number, path:number, actual_life:number, max_life:number, size:number, damage:number, actual_hunger:number, max_hunger:number, actual_thirst:number, max_thirst:number, is_travelling:number, home:string, actual_age:number, max_age:number, role:Role,
  // supply:Supply, 
  ...
  draw_xy(x,y):void {
    this._ctx.beginPath();
    this._ctx.fillStyle = 'navy';
    this._ctx.arc(x, y, 15, 0, (Math.PI/180)*360, false);this._ctx.stroke();
    this._ctx.fill();
  }
}